...
public class HierDirCtx extends HierCtx implements DirContext {
     ...
     * <tt>args</tt>.
     *
     * <code>{i}</code> specifies the <code>i</code>'th element from the array
     * <code>args</code> is to be substituted for the string "<code>{i}</code>".
     *
    ...
    /**
     * Finds the first occurrence of <tt>ch</tt> in <tt>val</tt> starting from
     * position <tt>start</tt>. It doesn't count if <tt>ch</tt>
    ...
    private static  Attributes deepClone(Attributes orig) throws NamingException {
        ...
        NamingEnumeration enumE = orig.getAll();
        while (enumE.hasMore()) {
            copy.put((Attribute) ((Attribute) enumE.next()).clone());
        }
